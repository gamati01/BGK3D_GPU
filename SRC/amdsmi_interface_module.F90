module amdsmi_interface
  ...
  implicit none
#ifdef ENERGY
  ! AMD SMI counterpart of nvml_interface_module.F90.
  ! The C wrapper (amdsmi_wrapper.c) exposes the very same symbols as the
  ! NVML wrapper, so these interfaces are identical: only the module name
  ! differs, allowing one or the other to be compiled depending on the GPU
  ! vendor.
  interface
